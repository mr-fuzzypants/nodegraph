...
def _json_safe_value(value: Any) -> Any:
    """
    Convert runtime port values into JSON-safe summaries.

    Imaging nodes keep heavy process-local objects (torch modules, tensors,
    tokenizers, PIL images) on their ports so downstream nodes can use them.
    Those objects must not be returned directly from REST endpoints.
    """
    ...
    if isinstance(value, dict):
        if all(isinstance(k, str) and isinstance(v, (str, int, float, bool, type(None))) for k, v in value.items()):
            return value

        summary: Dict[str, Any] = {
            "__nodegraph_opaque__": True,
            "type": "dict",
            "keys": list(value.keys()),
        }
        for key in ("samples", "IMAGE"):
            item = value.get(key)
            shape = getattr(item, "shape", None)
            if shape is not None:
                summary["shape"] = [int(dim) for dim in shape]
                summary["dtype"] = str(getattr(item, "dtype", ""))
                summary["device"] = str(getattr(item, "device", ""))
                break
        return summary
...
def _serialize_port(port: Any, connected: bool = False) -> Dict[str, Any]:
    ...
    return {
        ...
        "value": _json_safe_value(getattr(port, "value", None)),
        "connected": connected,
    }
...
def _serialize_tunnel_nodes(
    ...
    base_pos = positions.get(f"{network.id}:self", {"x": -300, "y": 140})
    input_pos = positions.get(
        f"{network.id}:in",
        {"x": base_pos.get("x", -300), "y": base_pos.get("y", 140)},
    )
    output_pos = positions.get(
        f"{network.id}:out",
        {"x": base_pos.get("x", -300), "y": base_pos.get("y", 140) + 220},
    )
...
def serialize_network(
    ...
    # Every subnetwork shows tunnel endpoints, even before ports are added.
    inputs_dict = getattr(network, "inputs", {})
    outputs_dict = getattr(network, "outputs", {})
    if parent_id is not None:
        nodes[0:0] = _serialize_tunnel_nodes(network, positions, graph)

    # Only include edges whose both endpoints belong to this network scope.
    edges: List[Dict[str, Any]] = []
    for e in graph.edges:
        if e.from_node_id not in node_id_set or e.to_node_id not in node_id_set:
            continue
        source_node_id = e.from_node_id
        target_node_id = e.to_node_id
        if e.from_node_id == network.id and e.from_port_name in inputs_dict:
            source_node_id = f"{network.id}:in"
        if e.to_node_id == network.id and e.to_port_name in outputs_dict:
            target_node_id = f"{network.id}:out"
        edges.append(
            {
