...
def create_socket_app(fastapi_app: Any, on_startup=None, on_shutdown=None) -> socketio.ASGIApp:
    """Wrap *fastapi_app* inside a Socket.IO ASGI application.

    ``on_startup`` and ``on_shutdown`` are forwarded to :class:`socketio.ASGIApp`
    so callers can run startup/shutdown hooks that the socket.io layer actually
    fires (unlike FastAPI lifespan events, which socket.io does **not** forward
    to the wrapped app).
    """
