...
# ── ImageGenExecNode ──────────────────────────────────────────────────────────
# Flow-control wrapper around DALL-E 3 image generation.
# Unlike ImageGenNode (pure data node), this node participates in the execution
# stack via exec/next control ports so it re-runs correctly inside loops even
# after being marked clean.
#
#  Control in  : exec
#  Data in     : prompt (STRING)  |  model (STRING, default dall-e-3)
#                size   (STRING, default 1024x1024)
#  Control out : next
#  Data out    : url (STRING)  |  revised_prompt (STRING)
#
# Tip: wire revised_prompt → PromptRefineExecNode.original_prompt so the next
# refinement iteration builds on DALL-E's own enriched version of the prompt.

@_safe_register("ImageGenExecNode")
class ImageGenExecNode(Node):
    def __init__(self, name: str, type: str = "ImageGenExecNode", **kwargs):
        super().__init__(name, type, **kwargs)
        self.is_flow_control_node = True
        ...
        self.inputs["prompt"].value          = "A beautiful landscape"
        self.inputs["model"].value           = "dall-e-3"
        self.inputs["size"].value            = "1024x1024"
        self.outputs["url"].value            = ""
        self.outputs["revised_prompt"].value = ""
...
# ── PromptRefineExecNode ──────────────────────────────────────────────────────
# Flow-control wrapper for LLM-based prompt combination.
# Participates in the execution stack via exec/next so it re-runs correctly
# inside loops without being skipped by the dirty-flag optimisation.
#
#  Control in  : exec
#  Data in     : original_prompt (STRING) — base prompt from prior generation
#                feedback        (STRING) — human's refinement directive
#                model           (STRING, default gpt-4o-mini)
#  Control out : next
#  Data out    : refined_prompt  (STRING)
#
# First-iteration behaviour: when original_prompt is empty (no prior image),
# the feedback is used directly as the full prompt without an LLM round-trip.

@_safe_register("PromptRefineExecNode")
class PromptRefineExecNode(Node):
    def __init__(self, name: str, type: str = "PromptRefineExecNode", **kwargs):
        super().__init__(name, type, **kwargs)
        self.is_flow_control_node = True

        self.inputs["exec"]            = InputControlPort(self.id, "exec")
        self.inputs["original_prompt"] = InputDataPort(self.id, "original_prompt", ValueType.STRING)
        self.inputs["feedback"]        = InputDataPort(self.id, "feedback",        ValueType.STRING)
        self.inputs["model"]           = InputDataPort(self.id, "model",           ValueType.STRING)
    ...
    async def compute(self, executionContext=None) -> ExecutionResult:
        ...
        ctx             = (executionContext or {}).get("data_inputs", {}) if executionContext else {}
        original_prompt = ctx.get("original_prompt", self.inputs["original_prompt"].value) or ""
        feedback        = ctx.get("feedback",        self.inputs["feedback"].value)        or ""
        model           = ctx.get("model",           self.inputs["model"].value)           or "gpt-4o-mini"

        # First iteration: no prior image exists — use feedback directly as the prompt.
        if not original_prompt.strip():
            refined = feedback.strip() or "a beautiful landscape"
            self.outputs["refined_prompt"].value = refined
            _fire(self, "NODE_DETAIL", detail={"refined_prompt": refined[:400], "mode": "passthrough"})
            result = ExecutionResult(ExecCommand.CONTINUE)
            result.data_outputs["refined_prompt"] = refined
            result.control_outputs["next"]        = True
            return result
        ...
        system = (
            "You are an expert image prompt engineer for DALL-E 3. "
            "Given an original image generation prompt and a refinement directive from the user, "
            "rewrite the prompt to incorporate the directive while preserving the core scene. "
            "Output ONLY the improved prompt — no explanation, no preamble, no quotes."
        )
        user_msg = f"Original prompt:\n{original_prompt}\n\nRefinement directive:\n{feedback}"

        _fire_edge(self, "feedback", "refiner_input", feedback[:120])
