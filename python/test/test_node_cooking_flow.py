...
from nodegraph.python.core.NodePort import InputControlPort, OutputControlPort, InputDataPort, OutputDataPort, ValueType, ExecutionContext
...
class FlowTestNode(Node):
    ...
    async def compute(self, executionContext=None):
        ...
        print(f"COMPUTING FLOW NODE: {self.id}")
        EXECUTION_LOG.append(self.id)
...
class DataTestNode(Node):
    ...
    async def compute(self, executionContext=None):
        global EXECUTION_LOG
        print(f"COMPUTING DATA NODE: {self.id}")
        EXECUTION_LOG.append(self.id)
...
@Node.register("MockSubnetNode")
class MockSubnetNode(NodeNetwork):
    def __init__(self, id, network=None, **kwargs):
        super().__init__(id, network=network)
        self.type = "MockSubnetNode"
        self.is_flow_control_node = True
        self.cooking_internally = False

    async def compute(self, executionContext=None):
        if self.cooking_internally:
             return ExecutionResult(ExecCommand.CONTINUE, next_node_ids=[])
             
        self.cooking_internally = True
        try:
            global EXECUTION_LOG
            EXECUTION_LOG.append(f"Subnet:{self.id}")

            # --- Propagate Input Data from Subnet Ports to Internal Nodes ---
            ...
            for edge in edges:
                internal_node = self.get_node(edge.to_node_id)
                if internal_node:
                    await self.cook_flow_control_nodes(internal_node)

            # Output Propagation (Tunneling Out)
            # Explicitly Pull data from internal nodes to Subnet Outputs (Boundary)
            # This is required for unconnected DataNodes (like InternalData -> Subnet.out)
            for edge in self.edges:
                if edge.to_node_id == self.id:
                     source_node = self.get_node(edge.from_node_id)
                     if source_node and edge.from_port_name in source_node.outputs:
                         val = source_node.outputs[edge.from_port_name].value
                         if edge.to_port_name in self.outputs:
                              self.outputs[edge.to_port_name].value = val

            control_outputs = {}
            if "finished" in self.outputs:
                control_outputs["finished"] = True
            
            # Next IDs resolution is handled by the runner if control_outputs are returned.
...
class TestNodeCookingFlow:
    ...
    def test_flow_linear(self):
        ...
        net.add_edge("Start", "next", "Middle", "exec")
        net.add_edge("Middle", "next", "End", "exec")
    ...
    def test_flow_branch(self):
        ...
        net.add_edge("A", "next", "B", "exec")
        net.add_edge("A", "next", "C", "exec")
    ...
    def test_mixed_data_flow(self):
        ...
        # Flow connection
        net.add_edge("Start", "next", "Consumer", "exec")
        
        # Data connection
        net.add_edge("Provider", "out", "Consumer", "data_in")
    ...
    def test_subnetwork_tunneling(self):
        ...
        net.add_edge("ExtData", "out", "subnet_stop", "tunnel_data")
        subnet_stop.add_edge("subnet_stop", "tunnel_data", "InternalFlow", "data_in")
        
        # 2. Flow: Subnet.tunnel_exec -> InternalFlow.exec
        # Note: We trigger the subnet directly via python for this test
        subnet_stop.add_edge("subnet_stop", "tunnel_exec", "InternalFlow", "exec")
    ...
    def test_nested_subnetwork_flow(self):
        ...
        subnet_a.add_edge("A", "exec", "B", "exec")
        subnet_b.add_edge("B", "exec", "Leaf", "exec")
    ...
    def test_nested_subnetwork_outputs(self):
        ...
        net.add_edge("Start", "next", "SubnetA", "exec")
        ...
        subnet_a.add_edge("SubnetA", "exec", "NodeA", "exec")
        subnet_a.add_edge("NodeA", "next", "SubnetB", "exec")
        
        # Level 2 (Inside SubnetB)
        node_b = subnet_b.createNode("NodeB", "FlowTestNode")
        
        # Connections inside B: SubnetB.exec -> NodeB
        subnet_b.add_edge("SubnetB", "exec", "NodeB", "exec")
    ...
    def test_subnet_output_flow_and_data(self):
        ...
        # Outer Connections
        net.add_edge("Start", "next", "Subnet", "exec")
        net.add_edge("Subnet", "finished", "End", "exec")
        net.add_edge("Subnet", "data_out", "End", "data_in")
        ...
        subnet.add_edge("Subnet", "exec", "InternalFlow", "exec")
        subnet.add_edge("InternalFlow", "next", "Subnet", "finished")

        # Data: InternalData -> Subnet.data_out
        subnet.add_edge("InternalData", "out", "Subnet", "data_out")
    ...
    def test_complex_hierarchical_data_and_flow(self):
        ...
        # Connections Level 0
        net.add_edge("RootStart", "next", "Subnet1", "exec")
        net.add_edge("RootData", "out", "Subnet1", "in_data")
        net.add_edge("Subnet1", "finished", "RootEnd", "exec")
        net.add_edge("Subnet1", "out_data", "RootEnd", "data_in")
        ...
        subnet1.add_edge("Subnet1", "exec", "L1_Start", "exec")
        subnet1.add_edge("L1_Start", "next", "Subnet2", "exec")
        subnet1.add_edge("Subnet2", "finished", "Subnet1", "finished")
    
        # Data: Subnet1.in_data -> L1_Op.a
        subnet1.add_edge("Subnet1", "in_data", "L1_Op", "a")
        # Data: L1_LocalData -> L1_Op.b
        subnet1.add_edge("L1_LocalData", "out", "L1_Op", "b")
        # Data: L1_Op.sum -> Subnet2.inner_data
        subnet1.add_edge("L1_Op", "sum", "Subnet2", "inner_data")
        # Data: Subnet2.result_data -> Subnet1.out_data
        subnet1.add_edge("Subnet2", "result_data", "Subnet1", "out_data")
        ...
        subnet2.add_edge("Subnet2", "exec", "L2_Start", "exec")
        subnet2.add_edge("L2_Start", "next", "L2_Consumer", "exec")
        subnet2.add_edge("L2_Consumer", "next", "Subnet2", "finished")
    
        # Data: Subnet2.inner_data -> L2_Consumer.data_in (Consumption)
        subnet2.add_edge("Subnet2", "inner_data", "L2_Consumer", "data_in")
    
        # Data: L2_Result -> Subnet2.result_data (Refers to outer port value set)
        subnet2.add_edge("L2_Result", "out", "Subnet2", "result_data")
    ...
    def test_sibling_subnet_data_handoff(self):
        ...
        producer.add_edge("P_Internal", "out", "Producer", "data_out")
        ...
        consumer.add_edge("Consumer", "data_in", "C_Check", "a")
        # Dummy value for b
        c_const = consumer.createNode("C_Const", "DataTestNode")
        c_const.outputs["out"].value = 0
        consumer.add_edge("C_Const", "out", "C_Check", "b")
        
        # 3. Root Connections
        start = net.createNode("Start", "FlowTestNode")
        
        # Flow: Start -> Producer -> Consumer
        net.add_edge("Start", "next", "Producer", "exec")
        net.add_edge("Producer", "finished", "Consumer", "exec")
        
        # Data: Producer -> Consumer
        net.add_edge("Producer", "data_out", "Consumer", "data_in")
        ...
        producer.add_edge("Producer", "exec", "P_Flow", "exec")
        producer.add_edge("P_Flow", "next", "Producer", "finished")
        
        # Consumer Internal Flow
        c_flow = consumer.createNode("C_Flow", "FlowTestNode")
        consumer.add_edge("Consumer", "exec", "C_Flow", "exec")
