...
class NodeNetwork(Node):
    ...
    async def compute_step(self, start_nodes: Optional[List[Node]]=None) -> Optional[List[Node]]:
        ...
        import asyncio
        assert(False), "Deprecated compute_step method, use compute with start_nodes instead"
        if start_nodes:
            ...
            tasks = [cur_node.compute(executionContext=ExecutionContext(cur_node).to_dict()) for cur_node in compute_nodes]
                ...
                if result and isinstance(result, ExecutionResult):
                    ...
                    if result.command == ExecCommand.CONTINUE:
                        if result.next_nodes:
                            ...
                            AssertionError("Node returned next_nodes and next_node_ids, should only use one")
            ...
            for cur_node in compute_nodes:        
                AssertionError("Legacy compute_step fallback should not be used if nodes return ExecutionResult")   
                ...
                for output_port in cur_node.get_output_control_ports():
                    # Check if the output port was activated by the node logic
                    if output_port.isActive():
                        # Get new Edges from Arena
                        outgoing_edges = self.get_outgoing_edges(cur_node.id, output_port.port_name)
                        for edge in outgoing_edges:
                            target_node = self.get_node(edge.to_node_id)
                            if target_node:
                                # Optional: We could check if target port is compatible, 
                                # but we assume the edge is valid.
    ...
    def get_inputoutput_control_ports(self) -> List[NodePort]:
        assert(False), "Deprecated get_inputoutput_control_ports method"
        # Using new Enums
    ...
    """
    async def compute_network(self, executionContext: Optional[Any]=None) -> None:
        
        assert(False), "Deprecated compute_network method, use compute with start_nodes instead"
        # Legacy compute method for networks.
        self.precompute()
    ...
    async def compute(self, start_nodes: Optional[List[Node]]=None, executionContext: Optional[Any]=None) -> None:
        ...
        if context:
            print("Context for network compute:", context)
            ExecutionContext(self).from_dict(context)

            #assert(False), "Cannot provide both executionContext and have NodeNetwork build one"
        """
        # what we are doing here is finding the internal nodes connected to the network's input ports.
        # we'll need to add these to the pending stack
        if next_nodes is None:
            # this will get all the inputs that are IO ports
            control_ports = self.get_inputoutput_control_ports()
            next_nodes = []
            for port in control_ports:
                # If network is triggered (tunnel in)
                # But here we check if *outgoing* connections from the network Input exist?
                # Yes, tunneling: Network Input -> Internal Node Input
                
                # Check edges
                outgoing_edges = self.get_outgoing_edges(self.id, port.port_name)
                for edge in outgoing_edges:
                     target_node = self.get_node(edge.to_node_id)
                     if target_node:
                         # Propagate activation to the internal node's input port
                         target_port = target_node.inputs.get(edge.to_port_name)
                         if target_port:
                             target_port.setValue(True)
                             
                         next_nodes.append(target_node)
                         logger.info(f"  Found start node from network exec port: {target_node.id}")
        ...
        for input_port in node.get_input_data_ports():
            ...
            for up_node in get_upstream_nodes_list:
                if up_node.isDirty() == False:
                    continue
                #if up_node.isFlowControlNode():
                #    pending_stack[node.id].append(up_node.id)
                print("***!! FNE Upstream Node:", up_node.id, "isDataNode:", up_node.isDataNode(), "isNetwork:", up_node.isNetwork())
                ...
                if up_node.isNetwork():
                    ...
                    #print(".... UPSTREAM NODE IS A NETWORK:", up_node.id)
                    #build_data_node_execution_stack(up_node, execution_stack, pending_stack)

                # couldn't this actually be simplified to at least? I think it can
                # if up_node.isDataNode() or up_node.isNetwork():
                #     pending_stack[node.id].append(up_node.id)
                #     self.build_flow_node_execution_stack(up_node, execution_stack, pending_stack)
        print("+++ After building data node stack:", execution_stack, pending_stack)
    ...
    def build_data_node_execution_stack(self, node: Node, execution_stack: List[str], pending_stack: Dict[str, List[str]]):
        ...
        for input_port in node.get_input_data_ports():
            ...
            for up_node in upstream_nodes:
                if up_node.isDirty() == False:
                    continue
                print("***!! DNE Upstream Node:", up_node.id, "isDataNode:", up_node.isDataNode(), "isNetwork:", up_node.isNetwork())
                if up_node.isDataNode(): 
                    pending_stack[node.get_path()].append(up_node.get_path())
                    self.build_data_node_execution_stack(up_node, execution_stack, pending_stack)
    ...
    """
    def build_flow_network_execution_stack(self, node: Node, execution_stack: List[str], pending_stack: Dict[str, List[str]]):
        # TODO: Is this even being used?
        #assert(("build_flow_network_execution_stack is deprecated, use build_flow_node_execution_stack instead")
        # get nodes connected to the network's exec input port
        assert(False), "build_flow_network_execution_stack is deprecated, use build_flow_node_execution_stack instead"
        control_ports = node.get_inputoutput_control_ports()
        for port in control_ports:
            # If network is triggered (tunnel in)
            # But here we check if *outgoing* connections from the network Input exist?
            # Yes, tunneling: Network Input -> Internal Node Input
            ...
            # Check edges
            outgoing_edges = node.network.get_outgoing_edges(node.id, port.port_name)
            for edge in outgoing_edges:
                 target_node = node.network.get_node(edge.to_node_id)
                 if target_node:
                     # Propagate activation to the internal node's input port
                     target_port = target_node.inputs.get(edge.to_port_name)
                     if target_port:
                     ...
                     # now build execution stack starting from this node
                     if target_node.isFlowControlNode():
                         node.network.build_flow_node_execution_stack(target_node, execution_stack, pending_stack)
                     elif target_node.isDataNode():
                         node.network.build_data_node_execution_stack(target_node, execution_stack, pending_stack)
    ...
    async def cook_network(self, node: Node) -> None:

        assert(False), "Deprecated cook_network method, use build_flow_network_execution_stack instead"
        execution_stack = []
        pending_stack = {}

        AssertionError("Deprecated cook_network method, use build_flow_network_execution_stack instead")
    ...
    async def cook_data_nodes(self, node):
        ...
        print("DATA NODE COOKING START")
        print("---------ALL NODES IN REGISTRY---------")
        for k,v in NodeNetwork.all_nodes.items():
            print("ALL NODES:", k, v.get_path())
        print("----------------------------------------")
        ...
        while execution_stack:  
            ...
            if cur_node and cur_node.isDataNode():
                ...
                #for output_name, output_value in result.data_outputs.items():
                #    out_port = cur_node.outputs.get(output_name)
                #    if out_port:
    ...
    def build_flow_network_execution_stack(self, node: Node, execution_stack: List[str], pending_stack: Dict[str, List[str]]):
        ...
        for input_port in node.get_input_data_ports():
            ...
            for up_node in get_upstream_nodes_list:
                ...
                if up_node.isNetwork():
                    self.build_flow_network_execution_stack(up_node, execution_stack, pending_stack)
                    #print(".... UPSTREAM NODE IS A NETWORK:", up_node.id)
                    #build_data_node_execution_stack(up_node, execution_stack, pending_stack)
    ...
    """
    async def cook_flow_network(self, node, executionContext: Optional[Any]=None) -> None:
        assert(False), "cook_flow_network is deprecated, use cook_flow_control_nodes instead"
        execution_stack = []
        
        pending_stack = {}


        # flow control nodes are forward cooked. We need to make sure that the
        # execution context can be built (with proper data inputs) when we cook them.
        # the cooked node then is responsible for determining the next nodes to cook.
        # and we build the execution and pending stacks accordingly.

        # we also need to make sure that data nodes upstream of flow control nodes
        # are cooked first. This means we may need to interleave data node cooking
        # with flow control node cooking.

        # tests should include a mix of data and flow control nodes.
        ...
        print("Pending Stack:", pending_stack)
        # Find all the eligible nodes (or networks) to start execution
        for node_id in list(pending_stack.keys()):
            deps = pending_stack[node_id]
            if len(deps) == 0:
                execution_stack.append(node_id)
        ...
        while execution_stack:
            print()
            print("BEGIN Execution Stack:", execution_stack)
            print("BEGIN Pending Stack:", pending_stack)
            cur_node_id = execution_stack.pop() 
            cur_node = node.network.get_node(cur_node_id)

            if cur_node: # and cur_node.isFlowControlNode():
                print(".   Cooking node:", cur_node_id)
                context = ExecutionContext(cur_node).to_dict()
                print(".       Context:", context)
                result = await cur_node.compute()
                print(".       Result:", result.command, result.next_node_ids, result.data_outputs, result.control_outputs)
                ...
                #TODO:  output values and updating ports.
                for output_name, output_value in result.control_outputs.items():
                    out_port = cur_node.outputs.get(output_name)
                    if out_port:
                        out_port.value = output_value
                        out_port._isDirty = False
                ...
                # now update pending stack with next nodes
                for next_node_id in result.next_node_ids:
                    next_node = node.network.get_node(next_node_id)
                    self.build_flow_node_execution_stack(next_node, execution_stack, pending_stack)
            ...
            for node_id in list(pending_stack.keys()):
                ...
                if len(deps) == 0:
                    execution_stack.append(node_id)
                    del pending_stack[node_id]
            print("POST pending stack after cooking node:", cur_node_id)
            print( "   Execution Stack:", execution_stack)
            print("   Pending Stack:", pending_stack)
 ...
 """
"""
def propagate_dirty_state_from_port_mine(self, source_node_id: str, port_name: str):
    assert(False), "propagate_dirty_state_from_port_mine is deprecated, use propagate_dirty_state_from_node instead"
   

    source_node = self.get_node(source_node_id)
    if not source_node:
        return
    source_port = source_node.outputs.get(port_name)
    if not source_port:
        return
    
    for down_port in self.get_downstream_ports(source_port):
        down_port.is_Dirty = True
        down_port.node._isDirty = True



def propagate_dirty_state_from_port(self, source_node_id: str, port_name: str):
    
    assert(False), "propagate_dirty_state_from_port is deprecated, use propagate_dirty_state_from_node instead"
   
    dirty_nodes = []
    visited = set()

    # Start from the source node's output ports
    source_node = self.get_node(source_node_id)
    if not source_node:
        return

    for output_port in source_node.get_output_data_ports():
        outgoing_edges = self.get_outgoing_edges(source_node.id, output_port.port_name)
        for edge in outgoing_edges:
            target_node = self.get_node(edge.to_node_id)
            if target_node and target_node.id not in visited:
                visited.add(target_node.id)
                dirty_nodes.append(target_node)

    # Propagate dirty state recursively
    while dirty_nodes:
        dirty_node = dirty_nodes.pop()
        dirty_node.markDirty()

        for output_port in dirty_node.get_output_data_ports():
            outgoing_edges = self.get_outgoing_edges(dirty_node.id, output_port.port_name)
            for edge in outgoing_edges:
                target_node = self.get_node(edge.to_node_id)
                if target_node and target_node.id not in visited:
                    visited.add(target_node.id)
