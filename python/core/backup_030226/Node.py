...
class ExecutionContext:
    ...
    def get_port_value(self, port) -> Dict[str, Any]:
        ...
        #print("GETTING PORT VALUE:", port.node.id, port.port_name, port.value, "EDGES:", incoming_edges)
        if not incoming_edges:
            print(f"No incoming connections for port '{port.port_name}' on node '{port.node.id}'")
            return None
        ...
        if source_port is None:
            print (f"Source port '{edge.from_port_name}' not found on node '{source_node.id}'")   
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {
            "uuid": self.node.uuid,
            "network_id": self.network.id if self.network else "",
        ...
        print("!!!!!!!! NODE EXECUTION CONTEXT", self.node.id, self.node.type, result)
        return result
    ...
    def register(cls, type_name: str) -> Callable[[Type['Node']], Type['Node']]:
        ...
        def decorator(subclass: Type['Node']) -> Type['Node']:
            if cls._node_registry.get(type_name):
                print("NOde registruer:", cls._node_registry)
