...
class Node(INode):
    ...
    def __init__(self, 
        ...
        # Set True on nodes whose compute() has side effects (LLM calls, HTTP
        # calls, human-in-the-loop waits) so the Executor can wrap them in a
        # DBOS durable step, giving exactly-once execution on process restart.
        self.is_durable_step: bool = False
        self._isDirty = True  
    ...
    @abstractmethod
    async def compute(self, executionContext: Optional[Any] = None) -> IExecutionResult:
        pass
    ...
    def to_dict(self) -> dict:
        """Serialise this node to a plain dict.
        Port values are preserved; the subclass type name is embedded so
        from_dict can restore the correct class via the registry."""
        return {
            "id":                   self.id,
            "name":                 self.name,
            "type":                 self.type,
            ...
            "is_flow_control_node": self.is_flow_control_node,
    ...
    def from_dict(cls, data: dict) -> "Node":
        ...
        Uses Node._node_registry (populated by @Node.register) so the correct
        subclass is instantiated.  Ports are created by the subclass __init__
        (via create_ports) and their saved values are then patched in."""
        ...
        for port_name, port_data in data.get("inputs", {}).items():
            if port_name in node.inputs:
                node.inputs[port_name].value    = port_data.get("value", node.inputs[port_name].value)
                node.inputs[port_name]._isDirty = port_data.get("is_dirty", True)
            else:
                # Port was removed from the schema — reconstruct from saved data
                node.inputs[port_name] = NodePort.from_dict(port_data)

        for port_name, port_data in data.get("outputs", {}).items():
            if port_name in node.outputs:
                node.outputs[port_name].value    = port_data.get("value", node.outputs[port_name].value)
                node.outputs[port_name]._isDirty = port_data.get("is_dirty", True)
            else:
                node.outputs[port_name] = NodePort.from_dict(port_data)
