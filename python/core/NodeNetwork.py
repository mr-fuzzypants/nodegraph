...
from .Types import NodeKind, ValueType
...
class NodeNetwork(Node):
    ...
    def to_dict(self) -> dict:
        """Serialise this network: all Node-level fields plus the inner Graph.

        Because all networks share a single flat Arena graph, we must restrict
        serialisation to DIRECT children only (nodes whose network_id == self.id).
        Edges are filtered to those where *both* endpoints either ARE this
        network (i.e. node_id == self.id, for tunnel connections) or are a
        direct child.  This prevents infinite recursion when nested subnets
        iterate the same Arena and encounter their parent again.

        Inner NodeNetworks are serialised recursively via their own to_dict().
        """
        d = super().to_dict()
        if self.graph:
            # Only direct children — avoids re-serialising grandchildren or parents
            direct_children = {
                nid: n
                for nid, n in self.graph.nodes.items()
                if n.network_id == self.id
            }

            def _belongs(node_id: str) -> bool:
                """True if node_id is this network or one of its direct children."""
                if node_id == self.id:
                    return True
                node = self.graph.nodes.get(node_id)
                return node is not None and node.network_id == self.id

            d["graph"] = {
                "nodes": {nid: n.to_dict() for nid, n in direct_children.items()},
                "edges": [
                    {
                    ...
                    if _belongs(e.from_node_id) and _belongs(e.to_node_id)
                ],
            }
        else:
            d["graph"] = {"nodes": {}, "edges": []}
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "NodeNetwork":
        """Reconstruct a NodeNetwork and its inner Graph from a serialised dict.
        Inner nodes are restored via PluginRegistry; NodeNetwork children are
        ...
        for _nid, node_data in graph_data.get("nodes", {}).items():
            if node_data.get("kind") == NodeKind.NETWORK.value:
                inner = NodeNetwork.from_dict(node_data)
            else:
                inner = Node.create_node(
                    node_data["name"],
                    node_data["type"],
                    network_id=node_data.get("network_id"),
                )
                inner.id = node_data["id"]
                for pname, pdata in node_data.get("inputs", {}).items():
                    if pname in inner.inputs:
                        inner.inputs[pname].value    = pdata.get("value", inner.inputs[pname].value)
                        inner.inputs[pname]._isDirty = pdata.get("is_dirty", True)
                for pname, pdata in node_data.get("outputs", {}).items():
                    if pname in inner.outputs:
                        inner.outputs[pname].value    = pdata.get("value", inner.outputs[pname].value)
                        inner.outputs[pname]._isDirty = pdata.get("is_dirty", True)
            graph.nodes[inner.id] = inner

        for edge_data in graph_data.get("edges", []):
            graph.add_edge(
                edge_data["from_node_id"],
                edge_data["from_port_name"],
                edge_data["to_node_id"],
                edge_data["to_port_name"],
            )
        ...
        # Restore tunnel ports (the network's own inputs/outputs)
        for port_name, port_data in data.get("inputs", {}).items():
            if port_name in net.inputs:
                net.inputs[port_name].value    = port_data.get("value", net.inputs[port_name].value)
                net.inputs[port_name]._isDirty = port_data.get("is_dirty", True)
            else:
                from .NodePort import NodePort
                net.inputs[port_name] = NodePort.from_dict(port_data)

        for port_name, port_data in data.get("outputs", {}).items():
            if port_name in net.outputs:
                net.outputs[port_name].value    = port_data.get("value", net.outputs[port_name].value)
                net.outputs[port_name]._isDirty = port_data.get("is_dirty", True)
            else:
                from .NodePort import NodePort
                net.outputs[port_name] = NodePort.from_dict(port_data)
    ...
    # Tunnel ports — appear on the subnet boundary (one side inside, one side outside).
    # They are regular typed ports with allow_multiple=True so fan-in/fan-out is
    # permitted and graph traversal passes through them transparently.
    def add_control_input_port(self, port_name: str) -> InputControlPort:
        ...
        port.allow_multiple = True
        self.inputs[port_name] = port
        return port

    def add_control_output_port(self, port_name: str) -> OutputControlPort:
        if port_name in self.outputs:
            raise ValueError(f"Control output port '{port_name}' already exists in node '{self.id}'")
        port = OutputControlPort(self.id, port_name)
        port.allow_multiple = True
        self.outputs[port_name] = port
        return port

    def add_data_input_port(self, port_name: str, data_type=None) -> InputDataPort:
        """Add a typed data tunnel-in port to the subnet boundary."""
        if port_name in self.inputs:
            raise ValueError(f"Data input port '{port_name}' already exists in node '{self.id}'")
        port = InputDataPort(self.id, port_name, data_type=data_type or ValueType.ANY)
        port.allow_multiple = True
        self.inputs[port_name] = port
        return port

    def add_data_output_port(self, port_name: str, data_type=None) -> OutputDataPort:
        """Add a typed data tunnel-out port to the subnet boundary."""
        if port_name in self.outputs:
            raise ValueError(
                f"Data output port '{port_name}' already exists in node '{self.id}'"
            )
        port = OutputDataPort(self.id, port_name, data_type=data_type or ValueType.ANY)
        port.allow_multiple = True
        self.outputs[port_name] = port
        return port
    ...
    def connect_output_to_refactored(self, source_node: Node, from_port_name: str, other_node: 'Node', to_port_name: str) -> Edge:
        """Connect source_node.from_port_name → other_node.to_port_name.

        Port-dict selection is determined *entirely* by the structural relationship
        between the two nodes, not by which dict happens to return non-None first.
        This eliminates the same-name ambiguity that the old fallback chain suffered
        from (e.g. a NodeNetwork with both inputs["data"] and outputs["data"]).

        Supported relationships
        ───────────────────────
        sibling→sibling   outputs["x"] → inputs["y"]   (normal data flow)
        parent→child      inputs["x"]  → inputs["y"]   (tunnel-in: value enters subnet)
        child→parent      outputs["x"] → outputs["y"]  (tunnel-out: value exits subnet)

        "Child → sibling" and unrelated nodes are rejected with a clear error.
        """

        # ── 1. Determine structuralch relationship ──────────────────────────────
        parent_to_child  = other_node.network_id == source_node.id
        child_to_parent  = source_node.network_id == other_node.id
        # Siblings share the same parent; guard against the two special cases above
        # so a node whose network_id IS the other's id isn't also counted as a sibling.
        sibling_to_sibling = (
            source_node.network_id == other_node.network_id
            and not parent_to_child
            and not child_to_parent
        )

        # ── 2. Explicit port-dict selection driven by relationship ─────────────
        if sibling_to_sibling:
            # Normal data edge: output → input
            from_port = source_node.outputs.get(from_port_name)
            to_port   = other_node.inputs.get(to_port_name)

        elif parent_to_child:
            # Tunnel-in: the network's INPUT port feeds a child node's INPUT port.
            # InputOutputDataPort lives in .inputs on a NodeNetwork.
            from_port = source_node.inputs.get(from_port_name)
            to_port   = other_node.inputs.get(to_port_name)

        elif child_to_parent:
            # Tunnel-out: a child node's OUTPUT port feeds the network's OUTPUT port.
            # InputOutputDataPort lives in .outputs on a NodeNetwork.
            from_port = source_node.outputs.get(from_port_name)
            to_port   = other_node.outputs.get(to_port_name)

        else:
            raise ValueError(
                f"No valid relationship between '{source_node.name}' (network_id={source_node.network_id}) "
                f"and '{other_node.name}' (network_id={other_node.network_id}, id={other_node.id}). "
                "Nodes must be siblings, parent→child, or child→parent."
            )

        # ── 3. Validate ports ─────────────────────────────────────────────────
        rel = (
            "sibling→sibling" if sibling_to_sibling
            else "parent→child (tunnel-in)" if parent_to_child
            else "child→parent (tunnel-out)"
        )
        if not from_port:
            raise ValueError(
                f"[{rel}] Port '{from_port_name}' not found on source '{source_node.name}'"
            )
        if not to_port:
            raise ValueError(
                f"[{rel}] Port '{to_port_name}' not found on target '{other_node.name}'"
            )
        if from_port.node_id == other_node.id:
            raise ValueError("Cannot connect a node's port to itself")

        # ── 4. Duplicate-connection guard ─────────────────────────────────────
        existing = self.graph.get_incoming_edges(other_node.id, to_port_name)
        if existing and not (getattr(to_port, "allow_multiple", False) or getattr(from_port, "allow_multiple", False)):
            raise ValueError(
                f"Input port '{to_port_name}' on '{other_node.name}' is already connected"
            )

        # ── 5. Wire the edge (Arena always stores from→to in execution order) ──
        return self.graph.add_edge(source_node.id, from_port_name, other_node.id, to_port_name)
    ...
    def connect_to_network_output(self, from_node: Node, from_port_name: str, to_port_name: str):
        ...
        assert(
            from_port.direction == PortDirection.OUTPUT or getattr(from_port, "allow_multiple", False)
        ), "Source port must be an OUTPUT or tunnel port"
    ...
    def connect_network_input_to(self, from_port_name: str, other_node: Node, to_port_name: str):
        ...
        assert(
            getattr(from_port, "allow_multiple", False)
        ), f"Port '{from_port_name}' is not a tunnel port (allow_multiple must be True)"
